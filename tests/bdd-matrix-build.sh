...
#   8. Rockchip 首启扩盘 preinit 钩子契约 v2 (B32-B44)
#
...
if [ "$has_openclash" = n ]; then
  skip "openclash 未启用, 跳过 kmod-ipt 防呆"
else
  missing=""
  grep -qxF 'CONFIG_PACKAGE_kmod-ipt-tproxy=y' config/common.config || missing="${missing}kmod-ipt-tproxy "
  grep -qxF 'CONFIG_PACKAGE_kmod-ipt-nat=y' config/common.config || missing="${missing}kmod-ipt-nat "
  grep -qxF 'CONFIG_PACKAGE_kmod-ipt-extra=y' config/common.config || missing="${missing}kmod-ipt-extra "
  if [ -z "$missing" ]; then
...
OUTDOOR_FEED_REVISION="653025b24bdc17c0eb989ac57ea0d8f134951476"
...
# 复用: 命令体 (剥注释行) — 多个断言据此判定"命令里"而非"注释里"出现某特征
hook_code() { grep -vE '^[[:space:]]*#' "$EXPAND_HOOK"; }
...
if hook_code | grep -qE '(^|[^a-zA-Z_])exit[[:space:]]+[1-9][0-9]*'; then
...
if hook_code | grep -qE '/dev/mmcblk[01]'; then
  bad "79_expand_rootfs 硬编码了 /dev/mmcblk0/1 (裸盘或分区) — 换盘即失效"
else
  ok "无硬编码设备名 (通过 block info + sysfs 动态探测真实节点)"
fi

scenario "B38 — v2 探测: 认 squashfs 组合分区 + sysfs 反推, 不依赖 loop0 (v1 死穴)"
# v1 死穴: preinit (80_mount_root 前) overlay loop 尚不存在, 找 f2fs/loop0 必落空。
# v2 改认 TYPE="squashfs" 组合分区, 经 sysfs partition 反推磁盘/分区号。
has_sq=$(hook_code | grep -cE 'TYPE="squashfs"' || true)
has_sysfs=$(hook_code | grep -cE '/sys/class/block' || true)
no_loop0=$(hook_code | grep -cE 'loop0' || true)
if [ "$has_sq" -gt 0 ] && [ "$has_sysfs" -gt 0 ] && [ "$no_loop0" -eq 0 ]; then
  ok "认 squashfs 组合分区 + sysfs 反推, 不依赖 loop0 (v1 死穴已除)"
else
  bad "探测契约破坏: squashfs(${has_sq}) sysfs(${has_sysfs}) loop0残留(${no_loop0})"
fi

scenario "B39 — offset 运行期动态算 (squashfs bytes_used 64K 对齐) + f2fs magic 预校验"
# offset 必须运行期算 (squashfs 每次编译大小变, 硬编码即错位毁数据), 与 fstools 算法一致;
# resize 前必须校验偏移处确为 f2fs (magic 0xF2F52010), 防 offset 错算摧毁数据。
has_align=$(hook_code | grep -cE 'ALIGN|65536' || true)
has_byteread=$(hook_code | grep -cE 'read_le|hexdump' || true)
has_magic=$(hook_code | grep -cE 'F2FS_MAGIC|4076150800' || true)
if [ "$has_align" -gt 0 ] && [ "$has_byteread" -gt 0 ] && [ "$has_magic" -gt 0 ]; then
  ok "offset 动态算 + 字节读 + f2fs magic 预校验俱全 (不硬编码、防错位毁数据)"
else
  bad "缺 64K 对齐(${has_align})/字节读(${has_byteread})/magic 校验(${has_magic})"
fi

scenario "B40 — 含 resize.f2fs + fsck.f2fs 调用 + offline (losetup 未挂载视图) invariant"
# v2 安全 invariant: f2fs 必须对未挂载视图 (losetup -o offset) 离线 resize, 不在活挂载上做。
has_resize=$(hook_code | grep -cE 'resize\.f2fs' || true)
has_fsck=$(hook_code | grep -c 'fsck\.f2fs' || true)
has_losetup_off=$(hook_code | grep -cE 'losetup .*-o' || true)
if [ "$has_resize" -gt 0 ] && [ "$has_fsck" -gt 0 ] && [ "$has_losetup_off" -gt 0 ]; then
  ok "resize.f2fs + fsck.f2fs + losetup -o 未挂载视图 (offline resize invariant)"
else
  bad "缺 resize.f2fs(${has_resize})/fsck.f2fs(${has_fsck})/losetup -o(${has_losetup_off})"
fi

scenario "B41 — S2 统一清理出口: 每个 losetup 句柄获取后所有出口都有 losetup -d (防僵尸 loop)"
# 悬空 loop 咬住 p2 → 80_mount_root 撞 EBUSY → fail-soft 变 fail-hard。契约: 拿到句柄后
# 每条出口 (校验不过/fsck 失败/resize 失败/成功) 都先 losetup -d。结构特征: losetup -d
# 出现次数 >= resize_overlay 内的退出分支数 (4)。这里以"出现 >= 4 次 losetup -d"为下界守护。
nd=$(hook_code | grep -cE 'losetup -d' || true)
if [ "$nd" -ge 4 ]; then
  ok "losetup -d 出现 ${nd} 次 (>=4, 每个 S2 出口都清理, 无僵尸 loop)"
else
  bad "losetup -d 仅 ${nd} 次 (<4) — 存在出口未清理, 僵尸 loop 会致 80_mount_root EBUSY"
fi

scenario "B42 — S1 防 bootloop: reboot 前对比分区表, 仅 p2 实质变大才 reboot"
# preinit 写不了持久标记, 若 sfdisk 没真改分区表又 reboot → 永久 bootloop。
# 契约: 脚本含分区尺寸对比 (newsize > 旧 size) 且 reboot 在该对比的真分支内。
has_cmp=$(hook_code | grep -cE 'newsize.*-gt|-gt[[:space:]]+"\$4"|\[ "\$newsize" -gt' || true)
has_reboot=$(hook_code | grep -cE 'reboot -f' || true)
# reboot 必须出现在 expand_partition (S1) 内且受 -gt 比较门控: 简化为两特征均present
if [ "$has_cmp" -gt 0 ] && [ "$has_reboot" -gt 0 ]; then
  ok "reboot 前对比分区表实变 (newsize -gt 旧值才 reboot, 防 bootloop)"
else
  bad "缺分区表实变对比(${has_cmp})或 reboot(${has_reboot}) — S1 可能无效重启致 bootloop"
fi

scenario "B43a — 五个 rockchip seed 均声明 f2fsck/sfdisk/losetup (v2: losetup 替代 partx-utils)"
expand_miss=0
for dev in r2s r3s r5s r5s-outdoor r68s; do
  seed="devices/$dev/seed.config"
  for pkg in f2fsck sfdisk losetup; do
...
[ "$expand_miss" = "0" ] && ok "r2s/r3s/r5s/r5s-outdoor/r68s 均声明 v2 扩盘三包 (f2fsck/sfdisk/losetup)"

scenario "B43b — partx-utils 已从 rockchip seed 移除 (v2 走 reboot 重读表, partx 成死包)"
# v2 用 reboot 让内核重读 MBR 分区表, 不再用 partx 在线刷新 → partx-utils 是死重, 删除。
partx_residual=0
for dev in r2s r3s r5s r5s-outdoor r68s; do
  if grep -qxF "CONFIG_PACKAGE_partx-utils=y" "devices/$dev/seed.config"; then
    bad "$dev/seed.config 仍含 CONFIG_PACKAGE_partx-utils=y (v2 死包, 应删除)"
    partx_residual=1
  fi
done
[ "$partx_residual" = "0" ] && ok "partx-utils 已从 5 个 rockchip seed 全部移除 (无死包)"

scenario "B43c — x86 seed 不声明扩盘包 (x86 无 eMMC/SD f2fs overlay 扩盘需求)"
x86_miss=0
for pkg in f2fsck sfdisk losetup partx-utils; do
  if grep -qxF "CONFIG_PACKAGE_${pkg}=y" devices/x86/seed.config; then
    bad "x86/seed.config 不应含 CONFIG_PACKAGE_${pkg}=y"
    x86_miss=1
  fi
done
[ "$x86_miss" = "0" ] && ok "x86 seed 未声明任何扩盘包 (正确排除)"

scenario "B44 — diy-part2 落位路径无 openwrt/ 前缀, 源用 MCPE_REPO_ROOT 绝对变量"
